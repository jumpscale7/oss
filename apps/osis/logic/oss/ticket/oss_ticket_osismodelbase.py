...
class oss_ticket_osismodelbase(j.code.classGetJSRootModelBase()):
    ...
    def new_message(self,value=None):

        if value==None:
            value2=j.core.codegenerator.getClassJSModel("osismodel","oss","message")()
        else:
            value2=value
        
        self._P_messages.append(value2)
        if self._P_messages[-1].__dict__.has_key("_P_id"):
            self._P_messages[-1].id=len(self._P_messages)
        return self._P_messages[-1]
